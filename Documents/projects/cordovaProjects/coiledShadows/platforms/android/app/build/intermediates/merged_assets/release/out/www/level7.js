...
var finish7 = false;
var daggers7 = parseInt(localStorage.getItem("daggers"));
var daggerThrown;
...
class level7 extends Phaser.Scene {
    ...
    this.gamePad = new GamePad({scene: this});
		...
		this.setListeners();
		daggers7 = parseInt(localStorage.getItem("daggers"));
		//PLAYER
    keyReached = false;
    ...
    this.daggerText = this.add.text(490,155, "Daggers: " + daggers7, {fontFamily: "AgoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0);
		const map = this.make.tilemap({key: "level7", tileWidth: 16, tileHeight:16});
    ...
    this.smoke.play("smokeAnims");
		...
		this.physics.add.collider(knight, batlayer, (p,b) => {
			...
			this.physics.add.overlap(p,batSprite, () => {
				if (i == 0){
					hp = hp-2;
          if (hp <= 0){this.sound.stopAll(); fullhp = stablehp; hp=fullhp; daggers7 = 0; rolling7 = false; this.scene.start(window.localStorage.getItem("level"))};
					this.hpText.destroy();
					this.hpText = this.add.text(285, 155, "HP: " + hp + "/" + fullhp, {fontFamily: "AGoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0);
					i = 1;
				}
		...
		this.physics.add.collider(knight, chestlayer, (p, c) => {
			...
			if (randomNum == 1){
				...
				this.physics.add.overlap(knight, subWeapon, (p, d) => {d.destroy(); daggers7 = daggers7 + 1; this.daggerText.destroy(); this.daggerText = this.add.text(490,155, "Daggers: " + daggers7, {fontFamily: "AgoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0)},null, this);
			};
			c.setVisible(false);
			c.x = 0;
			c.y = 0;
    ...
    this.time.addEvent({
      delay: 500,
      callback: ()=>{if (knight.y>750){this.sound.stopAll(); fullhp = stablehp; hp = fullhp; hp=fullhp; daggers7 = 0; rolling7 = false; this.scene.start(window.localStorage.getItem("level"))}},
      loop: true,
    });
		//CONTROLS
  ...
  jumpRight(){
	...
	down(){
		...
		if (finish7 == true){
			console.log("worked");
			finish7 = false;
			localStorage.setItem("daggers", daggers7);
			localStorage.setItem("hp", fullhp);
			localStorage.setItem("level", "level8");
			this.sound.stopAll();
  ...
  dodgeRoll(){
	...
	daggerThrow(){
		if(daggers7>0 && daggerThrown != true){
		daggerThrown = true;
