...
var finish3 = false;
var daggers3 = parseInt(localStorage.getItem("daggers"));
var daggerThrown;
...
class level3 extends Phaser.Scene {
    ...
    this.gamePad = new GamePad({scene: this});
		...
		this.setListeners();
		daggers3 = parseInt(localStorage.getItem("daggers"));
		//PLAYER
    keyReached = false;
    ...
    this.daggerText = this.add.text(490,155, "Daggers: " + daggers3, {fontFamily: "AgoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0);
    ...
    this.sound.play("theme_2",{loop: true, volume:.2});
		...
		this.physics.add.collider(knight, chestlayer, (p, c) => {
			...
			if (randomNum == 1){
				...
				this.physics.add.overlap(knight, subWeapon, (p, d) => {d.destroy(); daggers3 = daggers3 + 1; this.daggerText.destroy(); this.daggerText = this.add.text(490,155, "Daggers: " + daggers3, {fontFamily: "AgoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0)},null, this);
			};
			c.setVisible(false);
			c.x = 0;
			c.y = 0;
      ...
      let throwBone = this.time.addEvent({
        ...
        callback: ()=>{
          ...
          this.physics.add.collider(bone, knight, (b, p) => {
            ...
            this.hpText.destroy();
            this.hpText = this.add.text(285, 155, "hp: " + hp + "/" + fullhp, {fontFamily: "AGoblinAppears", fontSize: "16px"}).setScale(.33).setDepth(999).setScrollFactor(0);
            if (hp <= 0){this.sound.stopAll(); fullhp = stablehp; hp=fullhp; daggers3 = 0; rolling3 = false; this.scene.start(window.localStorage.getItem("level"))};
          }, null, this);
          this.time.addEvent({
            delay: 1499,
            callback: ()=>{
    ...
    this.time.addEvent({
      delay: 500,
      callback: ()=>{if (knight.y>750){this.sound.stopAll(); fullhp = stablehp; hp = fullhp; hp=fullhp; daggers3 = 0; rolling3 = false; this.scene.start(window.localStorage.getItem("level"))}},
      loop: true,
    });
		//CONTROLS
  ...
  jumpRight(){
	...
	down(){
		...
		if (finish3 == true){
			console.log("worked");
			finish3 = false;
			localStorage.setItem("daggers", daggers3);
			localStorage.setItem("hp", fullhp);
			localStorage.setItem("level", "level4");
			this.sound.stopAll();
  ...
  dodgeRoll(){
	...
	daggerThrow(){
		if(daggers3>0 && daggerThrown != true){
		daggerThrown = true;
